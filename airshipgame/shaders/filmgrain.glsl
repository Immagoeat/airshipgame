...
void main() {
    ...
    ivec2 size = imageSize(source_image);
    ...
    vec4 color = imageLoad(source_image, uv);   // always reads the CLEAN scene
    color.rgb += (noise - 0.5) * params.grain_amount;
    color.rgb = clamp(color.rgb, 0.0, 1.0);

    imageStore(output_image, uv, color);         // writes to a separate target
}